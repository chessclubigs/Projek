...
class User(db.Model, UserMixin):
    ...
    register_date = db.Column(db.DateTime, default=func.now(), nullable=False)
...
class Member(db.Model):
    ...
    tournaments = db.relationship("Tournament", secondary="participant", backref="members", lazy=True)
...
class Tournament(db.Model):
    ...
    tournament_date = db.Column(db.DateTime, default=func.now(), nullable=False)
    
    matches = db.relationship("Match", foreign_keys="Match.tournament_id", backref="tournament", lazy=True)
    participants = db.relationship("Member", secondary="participant", backref="tournaments", lazy=True)
...
class Match(db.Model):
    ...
    match_date = db.Column(db.DateTime, default=func.now(), nullable=False)
